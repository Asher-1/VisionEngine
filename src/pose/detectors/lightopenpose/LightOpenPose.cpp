...
namespace human_pose_estimation {
    ...
    int groupPeaksToPoses(const std::vector<std::vector<Peak> > &allPeaks,
        ...
        for (const auto &subsetI : subset) {
            ...
            int position = -1;
            mirror::PoseResult pose;
            pose.keyPoints.resize(keypointsNumber);
            for (const auto &peakIdx : subsetI.peaksIndices) {
                position++;
                pose.keyPoints[position].p = cv::Point2f(-1.0f, -1.0f);
                pose.keyPoints[position].prob = subsetI.score * std::max(0, subsetI.nJoints - 1);
                if (peakIdx >= 0) {
                    pose.keyPoints[position].p = candidates[peakIdx].pos;
                    pose.keyPoints[position].p.x += 0.5;
                    pose.keyPoints[position].p.y += 0.5;
                }
            }
...
void postProcess(const ncnn::Mat &pafs, const ncnn::Mat &heatmaps, std::vector<mirror::PoseResult> &poses,
...
//#pragma omp parallel for
    ...
    groupPeaksToPoses(
            ...
            foundMidPointsRatioThreshold, minJointsNumber, minSubsetScore, poses);
    ...
    for (int i = 0; i < poses.size(); i++) {
        mirror::PoseResult &pose = poses[i];
        for (int j = 0; j < keypointsNumber; j++) {
            if (pose.keyPoints[j].p.x == -1 || pose.keyPoints[j].p.y == -1) {
                continue;
            }
            poses[i].keyPoints[j].p.x = stride / upsample * pose.keyPoints[j].p.x / scale_x;
            poses[i].keyPoints[j].p.y = stride / upsample * pose.keyPoints[j].p.y / scale_y;
        }
    }
}
...
namespace mirror {
    LightOpenPose::LightOpenPose(PoseEstimationType type) : PoseDetector(type) {
        inputSize_ = cv::Size(456, 256);
    ...
    int LightOpenPose::loadModel(const char *root_path) {
        std::string pose_param = std::string(root_path) + modelPath_ + "/lightopenpose/human_pose_sim_opt.param";
        std::string pose_bin = std::string(root_path) + modelPath_ + "/lightopenpose/human_pose_sim_opt.bin";
        return Super::loadModel(pose_param.c_str(), pose_bin.c_str());
    }

#if defined __ANDROID__
    int LightOpenPose::loadModel(AAssetManager *mgr) {
        std::string pose_param = "models" + modelPath_ + "/lightopenpose/human_pose_sim_opt.param";
        std::string pose_bin = "models" + modelPath_ + "/lightopenpose/human_pose_sim_opt.bin";
        return Super::loadModel(mgr, pose_param.c_str(), pose_bin.c_str());
    }
#endif

    int LightOpenPose::detectPose(const cv::Mat &img_src, std::vector<PoseResult> &poses) const {
        ...
        // postprocess
        poses.clear();
        postProcess(pafs, heatmaps, poses, img_height, img_width, net_h, net_w);
        return 0;
    }
